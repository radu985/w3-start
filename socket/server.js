...
io.on("connection", socket => {
    ...
    socket.on("join-customer-chat", async userData => {
        ...
        let session = Array.from(chatSessions.values()).find(
        ...
        if (!session) {
            ...
            try {
                const apiUrl = process.env.NEXT_APP_URL || 10000
                const response = await fetch(
                    `${apiUrl}/api/chat/messages?sessionId=${session.id}`
                )
    ...
    socket.on("join-admin-chat", async userData => {
        ...
        console.log(
            "Sending active sessions to admin:",
    ...
    socket.on("join-session", async sessionId => {
        ...
        try {
            ...
            const apiUrl = process.env.NEXT_APP_URL || 10000
            const response = await fetch(
                `${apiUrl}/api/admin/chat/messages?sessionId=${sessionId}`
            )
    ...
    socket.on("delete-session-history", async data => {
        ...
        try {
            ...
            const apiUrl = process.env.NEXT_APP_URL || 10000
            const response = await fetch(
                `${apiUrl}/api/admin/chat/delete-history`,
                {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
            ...
            io.to("admin-room").emit("session-removed", {
                sessionId: data.sessionId,
            })
    ...
    function notifyAdminsOfSessions() {
        ...
        console.log(
            "Notifying admins of sessions. Total admins:",
            adminSockets.size,
            "Active sessions:",
            activeSessions.length
        )
